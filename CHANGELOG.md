...
- The test job now prints a coverage summary (`npx vitest run --coverage`) on the Node 20 leg, and the default `npx vitest run` now includes the integration suite replayed offline from the committed cassettes. No coverage fail-under gate, matching tango-python.
- The `conformance` job is now a hard gate that runs both conformance directions offline against the vendored contract on every PR — it no longer needs `TANGO_API_REPO_ACCESS_TOKEN` and no longer skips silently without it. A separate token-gated step diffs the vendored contract against the tango API repo's HEAD and emits a staleness warning (never a failure).
