...
export interface PaginatedResponse<T> {
   ...
   * Empty when every supplied agency token resolved cleanly — a non-empty list means part of the filter did not apply, so a small or empty `results` is not evidence that no such records exist.
   */
  agencyWarnings?: string[];
  /**
   * Agency tokens that matched no organization, keyed by filter name.
   * Empty when everything resolved.
   * Use this to fail loudly in a pipeline rather than treating a silently-narrowed result set as an answer.
   */
  unresolvedAgencyTokens?: Record<string, string[]>;
  /**
   * What each agency token actually resolved to, keyed by filter name.
   * Agency resolution is fuzzy, so a token can match an organization the caller did not intend — checking the resolved `name` is the only way to catch that from the client side.
   */
